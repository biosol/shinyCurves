...
server <- function(input, output) {
  ...
  ################## BIORAD - Fluorescent Probe ##################
  #####################################################
  ################## Read "Run Information" and "Data" Sheet: PROBE-Biorad ##########################
  ...
  ################## Standard Curve Tab (Table): PROBE-Biorad #######################
  stCurve <- function(){
  ...
  ################## Standard Curve Tab (Plots): PROBE-Biorad ##########################
  stdPlots <- function(){
    if (input$posctrlbiorad != 0){
      a <- stCurve()
      g <- readBiorad()[[3]]
  ...
  ################## Analysis Samples Tab: PROBE-Biorad ###################
  AnalysisSamples <- function(){
    ...
    if(input$dupsbiorad == TRUE & input$copiesforassig == TRUE){
      ...
      # If not, calculate mean, else Indetermined sample
      ...
      #### Indetermined for wrong controls (> maxctendoc)
      ...
      for (i in 1:nrow(ctchecks)){
        ...
        if (all(is.na(ctchecks[i,])) == TRUE){
          ...
          unc <- sum(str_count(ctchecks[i,], "Indetermined"), na.rm = TRUE)
          ...
          if (check >= input$numposgenbiorad & check >= pos){
            ...
            ctassig <- append(ctassig, "Indetermined")
        ...
        l <- apply(p, MARGIN = 1, function(x){
          if (x[["FinalCtCheck"]] == "Check copy number"){
            if (x[[paste(i,"(Copies)",sep="")]] == "-"){
              "-"
            } else if (x[[paste(i,"(Copies)",sep="")]] == "Indetermined"){
              "Indetermined"
      ...
      copyassig <- apply(copychecks, MARGIN = 1, function(x){
        if (x[["FinalCtCheck"]] == "Check copy number"){
          ...
          rep <- sum(str_count(cc, "Indetermined"), na.rm = TRUE)
  ...
  AnalysisSamplesDT <- function(){
    ...
    t <- defdt %>%
      ...
      formatStyle(
        columns = "Assignment",
        backgroundColor = styleEqual(c("Positive", "Negative", "Indetermined"), 
  ...
  ################## APPLIED QUANT STUDIO - Fluorescent Probe ##################
  ##############################################################################
  
  ################## Read Applied "Raw Data" Sheet: PROBE-Applied ###################
  ...
  ################## Sample Plate Tab: PROBE-Applied ##########################
  ...
  ################## Standard Curve Tab (Table): PROBE-Applied #####################
  stCurveApp <- function(){
  ...
  ################## Standard Curve Tab (Plots): PROBE-Applied ###########################
  ...
  ################## Analysis Samples Tab: PROBE-Applied ######################
  ...
  ################## AMPLIFICATION CURVES - Fluorescent Probe ##################
  ############################################
  ...
  ################## Gene List from input CSVs: PROBE ###############
  geneList <- function(){
    dat <- input$taqmancsv
  ...
  ################## Combine ID well and ID results: PROBE ###################
  ...
  generalPlots <- function(){
    ...
    for (i in 1:length(genes)){
      ...
      color_map <- c(Positive="green3", Negative="tomato", Indetermined="blue")
      # Launch plot
      p <- plot_ly()
      interp <- c(Positive = 0, Negative = 0, Indetermined = 0)
  ...
  indetPlots <- function(){
    ...
    for (i in 1:length(genes)){
      ...
      ## one-by-one 
      merge_pn<-merge[merge$Interpretation!="Indetermined",]
      merge_r<-merge[merge$Interpretation=="Indetermined",]
      ...
      if (length(merge_r_persample) == 0){
        ...
        for (z in 1:length(merge_r_persample)){
          ...
          pltList[[pltName]] <- ggplot(data = merge_pn, aes(x=Cycles, y=RFU, by=Well2, color=Interpretation, text = paste("Sample:", ID))) +
            ...
            scale_color_manual(values = c("Negative"="tomato","Positive"="green3","Indetermined"="blue"))+
            ylim(-100, lr[[1]])+
            theme_minimal()+
  ...
  output$indetplots <- renderUI({
    ...
    tabs <- lapply(genes, function(x){
      tabPanel(
        ...
        uiOutput(paste("down",x,sep="")), 
        plotlyOutput(paste("indet",x,sep=""), height = "1000px")
      )
    })
    ...
    ## Message for no "Indetermined" samples
    lapply(genes, function(x){
  ...
  ################### MELTING CURVE - Intercalating dye #########################
  ##################################################################
  ...
  ################## Melting Curve Plots: DYE ##################
  # Function to Indetermined temperature columns
  ## 5) Plot  TM curves
  # a) Define function
  ...
  ################## BIORAD - Intercalating dye ##################
  ###################################################
  
  ################## Read ID Well Tab: DYE-Biorad ##############################
  ...
  ################## Read "Run Information" and "Data": DYE-Biorad ##########
  # This function allows the user to upload either 1 Excel with 2 sheets (Data and Run Information)
  # or to upload 2 independent CSVs with the Run Information and Quantification Summary (with Sample,
  # Fluor, Target, Cq, etc columns)
  ...
  ################## Standard Curve Tab (Table): DYE-Biorad #####################
  ...
  ################## Standard Curve Tab (Plots): DYE-Biorad ###########################
  ...
  ################## Analysis Samples Tab: DYE-Biorad ######################
  AnalysisSamplesSYBR <- function(){
    ...
    if(input$dupssybr == TRUE & input$copiesforassigsybr == TRUE){
      ...
      controlcq <- sapply(mean_cq_merged[,paste(input$endocsybr,"(MeanCt)", sep = "")], function(x){
        if (is.na(x) == TRUE){
          NA
        } else if (as.character(x) == "Indetermined"){
          "Indetermined"
  ...
  ################ APPLIED - Intercalating dye ##############
  ##############################################
  
  ################## Read ID Well Tab: DYE-Applied ##############################
  ...
  ################## Read Run Info and Raw Data: DYE-Applied #####################
  ## tbl_list will contain:
  ## 1) Df with all data
  ## 2) Df only with IDs to analyse (taken from ID_well from melting curves)
  ## 3) Run Information
  ...
  ################## Standard Curve Tab (Table): DYE-Applied #####################
  ...
  ################## Standard Curve Tab (Plots): DYE-Applied ###########################
  ...
  ################## Analysis Samples Tab: DYE-Applied ######################
